 ...
 * {@code IDispositionDAO}; por otra parte, esta clase hereda indirectamente de
 ...
 * las entidades pertencientes a la clase {@code Disposition}, son solo aquellos
 * que se han definido en la interfaz o prototipo {@code IDispositionDAO}; de
 * ese modo, esta clase se convierte en un D.A.O. <i>Data Access Object</i>,
 * para gestionar los datos de las entidades {@code Disposition}.
 * 
 * @since JDK1.7
 * 
 * @version 1.0
 ...
 */
public class DispositionDAOImpl extends AbstractEntityDAO implements
		IDispositionDAO {
	 ...
	 * constructor padre.
	 */
	public DispositionDAOImpl() {
		super();
	}

	@Override()
	public Disposition find(String dispositionKey)
			throws MANDevicesLendingDAOException {

		return ((Disposition) super.find(Disposition.class, dispositionKey));
	}

	@Override()
	public List<Disposition> findAll() throws MANDevicesLendingDAOException {
		List<Disposition> dispositionsFound = new ArrayList<Disposition>();
		List<IEntityContext> entitiesContextFoundList = super
				.findAll(Disposition.class);

		for (IEntityContext entityContext : entitiesContextFoundList) {
			dispositionsFound.add((Disposition) entityContext);
		}

		return (dispositionsFound);
	}

	@Override
	public Disposition save(Disposition disposition)
			throws MANDevicesLendingDAOException {
		String dispositionKey = (String) super.save(disposition);

		return ((dispositionKey != null) ? disposition : null);
	}
}