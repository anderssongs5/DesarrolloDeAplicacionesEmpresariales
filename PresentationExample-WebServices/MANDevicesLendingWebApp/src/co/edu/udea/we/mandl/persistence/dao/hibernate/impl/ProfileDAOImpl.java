 ...
 * {@code IProfileDAO}; por otra parte, esta clase hereda indirectamente de
 ...
 * las entidades pertencientes a la clase {@code Profile}, son solo aquellos que
 * se han definido en la interfaz o prototipo {@code IProfileDAO}; de ese modo,
 * esta clase se convierte en un D.A.O. <i>Data Access Object</i>, para
 * gestionar los datos de las entidades {@code Profile}.
 * 
 * @since JDK1.7
 * 
 * @version 1.0
 ...
 */
public class ProfileDAOImpl extends AbstractEntityDAO implements IProfileDAO {
	...
	@Override()
	public Profile find(String profileKey) throws MANDevicesLendingDAOException {

		return ((Profile) super.find(Profile.class, profileKey));
	}

	@Override()
	public List<Profile> findAll() throws MANDevicesLendingDAOException {
		List<Profile> profilesFound = new ArrayList<Profile>();
		List<IEntityContext> entitiesContextFoundList = super
				.findAll(Profile.class);

		for (IEntityContext entityContext : entitiesContextFoundList) {
			profilesFound.add((Profile) entityContext);
		}
	...
	@Override
	public Profile save(Profile profile) throws MANDevicesLendingDAOException {
		String profileKey = (String) super.save(profile);
